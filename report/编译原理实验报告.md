...
赋值语句除了简单的赋值语句以外还包括表达式的计算及赋值，其中优先级为()>*>-(负号)>+，为实现优先级的表示，我们采用不同的表达式表示语句，并且以表达式的嵌套关系实现优先级。
...
为保证标号语句和goto语句能够同时进行测试，加入了";"和"{}"的语法翻译，同时设置m方便回填。
...
本次实验为中间代码生成器编写了30个测试，另有2个语法分析器的测试，运行`pytest`可以得到结果为测试全部通过。
...
| e := - ( a + b ) * ( c + d ) + ( a + b + c )                 | temp0 := a + b<br /> temp1 := c + d<br /> temp2 := temp0 * temp1<br />temp3 := uminus temp2<br /> temp4 := a + b<br /> temp5 := temp4 + c<br /> temp6 := temp3 + temp5<br /> e := temp6 |
...
| if not ( a > b and c or d ) or not e <> f and not g then h := i | j>, a, b, 2<br />j, -, -, 4<br />jnz, c, -, 6<br />j, -, -, 4<br />jnz, d, -, 6<br />j, -, -, 10<br />j<>, e, f, 11<br />j, -, -, 8<br />jnz, g, -, 11<br />j, -, -, 10<br />h := i |
| if not ( - a + b ) * c and ( d or a + b > d + e ) then b := c | temp0 := uminus a<br />temp1 := temp0 + b<br />temp2 := temp1 * c<br />jnz, temp2, -, 12<br />j, -, -, 5<br />jnz, d, -, 11<br />j, -, -, 7<br />temp3 := a + b<br />temp4 := d + e<br />j>, temp3, temp4, 11<br />j, -, -, 12<br />b := c |
...
为了保证代码可以在不同的平台和环境中稳定复现，本项目使用了pipenv进行环境管理，通过`pipenv sync`指令即可安装和实验环境完全相同的环境，并提示潜在的环境差异（如python版本、操作系统不同）。此后使用`pipenv shell`指令可以在此环境中进行代码的使用和测试。根据测试，本项目可以在linux环境和项目的云平台上运行。云平台上运行情况如图
...
本项目不包括生成LR分析表实验内容，因此我们在项目中集成了开源的LR分析表生成器，生成LR分析器，用于后续的测试。通过运行tablegen.py可以根据语法生成对应的LR分析表，供LR分析器使用。

### 平台使用反馈

我们在平台上配置了环境并运行了本项目的测试，测试结果通过。

![云平台截图](res/reproduce.png)
...
本次实验所要求的实验内容为根据输入的语法树，进行自下而上的归约，在归约的过程中生成中间代码。
